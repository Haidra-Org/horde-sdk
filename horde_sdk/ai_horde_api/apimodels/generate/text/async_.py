...
class _BasePayloadKoboldMixin(HordeAPIData):
    """Mixin containing common fields for text generation payloads."""
...
class ModelPayloadRootKobold(_BasePayloadKoboldMixin):
    """Base class for user text generation requests ('payloads')."""
...
class ModelGenerationInputKobold(ModelPayloadRootKobold):
    """Represents the `params` field in the `/v2/generate/text/async` endpoint."""
...
class TextGenerateAsyncRequest(
    ...
    """Request to start a text generation job.

    Upon a successful response, you should poll the status of the job using the
    check endpoint. See :class:`horde_sdk.ai_horde_api.apimodels.generate.check.TextGenerateStatusRequest` for more
    information.
